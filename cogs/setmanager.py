...
class SetManager(commands.Cog):
    ...
    async def SetManager(self, interaction: discord.Interaction, member: discord.Member, team: discord.Role):
        ...
        cursor.execute(
            """
            INSERT INTO signings (guildid, userid, teamid, manager, signed)
            VALUES (?, ?, ?, ?, ?)
            """,
            (
        ...
        await interaction.response.send_message(f"{member.mention} has been assigned as the manager of {team.mention}")
