...
class Scouting(commands.Cog):
    ...
    @app_commands.command(name="scouting", description="Scout players for your team")
    async def Scouting(self, interaction: discord.Interaction, positions: str, regions: str):
        ...
        cursor.execute("SELECT scoutingchannelid FROM guild_config WHERE guildid = ?", (interaction.guild.id,))
        scoutingid = cursor.fetchone()
        conn.commit()

        if scoutingid is None:
            conn.close()
            return

        scoutingchannel = await interaction.guild.fetch_channel(scoutingid[0])
        ...
        embed.add_field(name="Team Scouting", value=f"{self.teamrole.mention} is looking for players", inline=False)
        embed.add_field(name="", value=f"> • Manager - {interaction.user.mention} {interaction.user.name}", inline=False)
        embed.add_field(name="", value=f"> • Positions - {positions}", inline=False)
        embed.add_field(name="", value=f"> • Regions - {regions}", inline=False)
        
        aaaa = await scoutingchannel.send(embed=embed)
        
        await interaction.response.send_message(f"Scouting message sent: {aaaa.jump_url}")
